...
def main():
    ...
    for c in range(totalStudents):
        ...
        numberdata = int(d30[c]) # search for this data
        
        if numberdata < 10 and d33[c] > 9:
            yesPassed += 1
        if numberdata  < 10 and d33[c] < 10:
            yesFailed += 1
        if numberdata > 15 and d33[c] > 9:
            noPassed += 1
        if numberdata > 15 and d33[c] < 10:
            noFailed += 1
#yes = low absence , no = high abscence"
