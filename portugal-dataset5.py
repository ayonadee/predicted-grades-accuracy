...
def main():
    ...
    print("all data included, none missing")
    ...
    yesPassed = 0 #students who passed and study for more than 9 hours
    yesFailed = 0
    noPassed = 0
    noFailed = 0
    ...
    for c in range(totalStudents):
        ...
        if numberdata < 3 and d33[c] > 9: #all students who studied for less than 9 hours and passed
            yesPassed += 1
        if numberdata  < 3 and d33[c] < 10:
            yesFailed += 1
        if numberdata > 2 and d33[c] > 9:
            noPassed += 1
        if numberdata > 2 and d33[c] < 10:
            noFailed += 1
#yes = university, no = no university"
#data check to analysie the fairness of the data
            
    print("weeklystudytime")
